...
import it.mpace.thomas.actors.Butterfly;
...
import it.mpace.thomas.actors.Hunchback;
...
public class Level4Screen extends LevelScreen implements Screen {
	...
	private float[] holesX = { LevelConstants.BUTTERFLY_HOLE_1_X, LevelConstants.BUTTERFLY_HOLE_2_X,
			LevelConstants.BUTTERFLY_HOLE_3_X, LevelConstants.BUTTERFLY_HOLE_4_X };
	private float[] holesY = { LevelConstants.BUTTERFLY_HOLE_1_Y, LevelConstants.BUTTERFLY_HOLE_2_Y,
			LevelConstants.BUTTERFLY_HOLE_3_Y, LevelConstants.BUTTERFLY_HOLE_4_Y };
	private float[] holeCooldowns = { 0, 0, 0, 0 }; // Timer per ogni buco
	private final float SPAWN_COOLDOWN = 1.5f; // Tempo di attesa tra una farfalla e l'altra per buco
	private final float TRIGGER_DISTANCE = 150f; // Distanza a cui si attiva il buco

	public Level4Screen(ThomasMain g) {
		super(g);
		// Inizializzazione entità
		minions = new Array<>();
		// ... setup camera e batch ...
		background = new Texture("kungfum_4_floor.png");
		this.levelInfo = new LevelInfo();
		this.levelInfo.setFloor(4);
		this.levelInfo.setLevelBeginX(LevelConstants.FORTH_FLOOR_LEFT);
		this.levelInfo.setRespawnX(LevelConstants.FORTH_FLOOR_LEFT_START); // Punto di partenza
		this.levelInfo.setGoalX(LevelConstants.FORTH_FLOOR_RIGHT_STAIR); // Punto di arrivo
		this.levelInfo.setGroundY(LevelConstants.FORTH_FLOOR_GROUND_Y);
		this.levelInfo.setGripperSpeed(LevelConstants.FORTH_FLOOR_GRIPPER_SPEED);
		this.levelInfo.setKnifeThrowerSpeed(LevelConstants.FORTH_FLOOR_KNIFE_THROWER_SPEED);
		this.levelInfo.setKnifeSpeed(LevelConstants.FORTH_FLOOR_KNIFE_SPEED);
		this.levelInfo.setKnifeInterval(LevelConstants.FORTH_FLOOR_KNIFE_INTERVAL);
		this.levelInfo.setMaxKnifeThrowers(LevelConstants.MAX_KNIFE_THROWERS_FORTH_FLOOR);
		this.levelInfo.setMaxEnemies(LevelConstants.MAX_ENEMIES_FORTH_FLOOR);
		this.levelInfo.setSpawnOffset(LevelConstants.SPAWN_OFFSET_FORTH_FLOOR);
		this.levelInfo.setCameraY(LevelConstants.FORTH_FLOOR_Y);
		this.levelInfo.setSpawnInteval(LevelConstants.SPAWN_INTERVAL_FORTH_FLOOR);
		this.levelInfo.setCeilingY(LevelConstants.FORTH_FLOOR_CEILING);
		this.levelInfo.setBossDistance(LevelConstants.FORTH_FLOOR_BOSS_DISTANCE);
		camera.position.x = this.levelInfo.getLevelBeginX();
		camera.position.y = this.levelInfo.getCameraY();
		trapdoor = new Trapdoor(LevelConstants.FORTH_FLOOR_TRAPDOOR_X, LevelConstants.FORTH_FLOOR_TRAPDOOR_Y, true);
		player = new Player(levelInfo.getLevelBeginX(), levelInfo.getGroundY(), this, levelInfo);
		System.out.println(levelInfo.toString());
		boss = new Hunchback(LevelConstants.FORTH_FLOOR_RIGHT_STAIR, LevelConstants.FORTH_FLOOR_GROUND_Y, false, this);
	...
	protected void handleBoss(float dt) {
		...
		if (!boss.isActive() && dist < 220) { // Raggio di attivazione
			...
			// System.out.println("BOSS QUARTO PIANO SVEGLIATO!");
		}
		if (boss.isActive()) {
			if (boss instanceof Hunchback) {
				// ((Hunchback) boss).update(dt, player, boomerangs);
	...
	protected void handleEnemies(float dt) {
		...
		for (int i = enemies.size - 1; i >= 0; i--) {
			...
			if (isBossActive && !(e instanceof Hunchback)) {
				e.flee();
			}
	...
	public void startLevelTransition(float dt) {
		// Previously we checked boss.isActive() which can remain true while the boss is
		// in dying physics
		// (flying off-screen). Use boss.getState() == DEAD to trigger the transition as
		// soon as the boss
	...
	protected void handleSpawning(float dt) {
		// 1. Decrementa TUTTI i cooldown dei buchi a ogni frame (INDIPENDENTEMENTE
		// dalla distanza)
		for (int i = 0; i < holeCooldowns.length; i++) {
			if (holeCooldowns[i] > 0) {
				holeCooldowns[i] -= dt;
			}
		}
		...
		if (spawnTimer >= this.levelInfo.getSpawnInteval()) {
			...
			Enemy newEnemy;
			if (player.position.x < LevelConstants.FORTH_FLOOR_HALF) {
				// 2. LOGICA DEI BUCHI (Butterfly Zone)
				for (int i = 0; i < holesX.length; i++) {
					// Aggiorna il cooldown del buco specifico
					if (holeCooldowns[i] > 0) {
						holeCooldowns[i] -= dt;

					}

					// Se Thomas è vicino al buco E il cooldown è terminato
					float distanceToHole = Math.abs(player.position.x - holesX[i]);
					if (distanceToHole < TRIGGER_DISTANCE && holeCooldowns[i] <= 0) {

						// Creiamo la farfalla
						Butterfly b = new Butterfly(holesX[i], holesY[i], this);

						// Decidiamo la direzione: la farfalla punta Thomas
						b.facingRight = (b.position.x < player.position.x);

						enemies.add(b);

						// Reset del cooldown per questo specifico buco
						holeCooldowns[i] = SPAWN_COOLDOWN + MathUtils.random(0.5f);
						;

						// Se abbiamo raggiunto il limite con questa farfalla, usciamo dal loop dei
						// buchi
						if (enemies.size >= this.levelInfo.getMaxEnemies())
							break;
					}
	...
	private void goToNextFloor() {
		...
		game.setScreen(new Level5Screen(this.game)); // Dovrai creare questa classe
	}
	...
	private void handleCrows(float dt) {
		for (int i = crows.size - 1; i >= 0; i--) {
			Crow c = crows.get(i);
			c.update(dt);

			// Collisione con Thomas
			if (c.hitbox.overlaps(player.hurtbox) && player.currentState != Player.State.DEAD) {
				// Se Thomas è accovacciato, il corvo passa sopra (schivata)
				if (player.currentState != Player.State.CROUCHING) {
					player.takeHit(10f, this, c.position.x, c.position.y);
					c.active = false;
				}
			}

			// Pulizia
			if (!c.active || Math.abs(c.position.x - player.position.x) > 300) {
				crows.removeIndex(i);
			}
		}
	}

	// Nel metodo update() chiama questo:
	private void handleFlames(float dt) {
		for (int i = flames.size - 1; i >= 0; i--) {
			MagicFlame f = flames.get(i);
			f.update(dt);

			if (f.hitbox.overlaps(player.hurtbox) && player.currentState != Player.State.DEAD) {
				// La fiamma colpisce sempre Thomas a meno che non salti quella orizzontale
				// o si sposti da quella diagonale.
				player.takeHit(15f, this, f.position.x, f.position.y);
				f.active = false;
			}
...
//        PlayerRes.dispose();
	...
	@Override
	public void show() {
		trapdoor.position.x = LevelConstants.FORTH_FLOOR_TRAPDOOR_X;
		player.position.x = LevelConstants.FORTH_FLOOR_LEFT_HOLE;
